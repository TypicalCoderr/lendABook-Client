...
import "../reserveBooks/reserveBookModal.scss";

import { addToBookCart } from "../../redux/actions/cartActions";

import { removeBookfromFavorites } from "../../redux/actions/dataActions";
...
function ViewFavorite(props) {
  ...
  const handleRemoveFavorite = async () => {
    ...
    let result = await props.removeBookfromFavorites(book.ISBN);
  ...
  const favRemoveToaster = () => {
    toast.success("book was removed from favorites!", {
      position: toast.POSITION.TOP_RIGHT,
      autoClose: 4000,
      draggable: false,
    });
...
ViewFavorite.propTypes = {
  ...
  user: PropTypes.object,
  removeBookfromFavorites: PropTypes.func.isRequired,
};
...
export default connect(mapStateToProps, mapActionsToProps)(ViewFavorite);
