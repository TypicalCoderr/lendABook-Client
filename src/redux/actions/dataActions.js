...
export const updateMovie = (movie) => async (dispatch) => {
  ...
  try {
    let results = await axios.post("/movies/update-movie", movie);
    if (results) return true;
...
export const removeBookfromFavorites = (ISBN) => async (dispatch) => {
  dispatch({ type: LOADING_UI });
  try {
    let results = await axios.delete(`/bookFavorites/${ISBN}`);
    dispatch(getMyBookFavorites());
    dispatch({ type: SET_BOOK_FAVORITES, payload: null });
    dispatch({ type: CLEAR_ERRORS });
    console.log(results);
    //Prevent modal from closing after errors are displayed
    if (results.data.message === "book removed from your favorites")
      return true;
  } catch (error) {
    dispatch({
      type: SET_ERRORS,
