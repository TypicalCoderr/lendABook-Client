...
export const registerUser = (user_data, history) => async (dispatch) => {
  ...
  try {
    ...
    // let props.accountCreated = true;
    // let accountCreated;
    // if (result.data.message === "Account created successfully, please login!")
    //   accountCreated = true;
...
export const updateUserdetails = (data) => async (dispatch) => {
  ...
  try {
    // console.log("sss" + formData);
    let results = await axios.post("/user/updateDetails", data);
    dispatch(getUserData());
    dispatch({ type: CLEAR_ERRORS });
    dispatch({ type: STOP_LOADING_UI });
    if (results.data.message === "User updated successfully") return true;
  } catch (error) {
    dispatch({
      type: SET_ERRORS,
      payload: error.response?.data,
